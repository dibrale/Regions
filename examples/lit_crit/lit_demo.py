...
This demo is intended to demonstrate how the system can be used to generate a literary
commentary on a poem. The structure includes:

  - 5x Region instances
  - 1x RAGRegion instance
  - 1x ListenerRegion instance

The system runs in seven execution layers for two rounds of analysis. No messages are cleared from
Region instance reply buffers between rounds. The ListenerRegion will open a GUI to snoop on
communications. Make sure to set your server details below. If the execution plan is not suitable
for your setup, import 'lit_demo_state.json' via the GUI and adjust these to your liking. There
are additional code blocks below that can be substituted in to run the 'Commentate' region alone
to compare with one-shot output.
...
async def main():
    ...
    llm_4b_b = LLMLink('192.168.1.232:5002', params=params['llm_params'])
    ...
    # Note: the original songbook is left out of the distributed code at the request of the artist,
    # but they are happy for you to hack the included database if you wish.
    ...
    # 2. Load the poem
    with open('i_know_poem.txt', encoding ="utf-8") as f:
        poem = ''.join(f.readlines())

    # 3. Load regions and execution plan from JSON
    r.load("lit_demo_regions.json")
    o.load("lit_demo_executions.json")

    # 4. Build-verify the regions and verify the execution plan, cross-verify
    # infrastructure instances. This should produce six warnings, since we
    # have not specified RAGs or LLMs yet.
    verify(r, o, p)

    # 5. Assign RAGs and LLMs to regions
    r['Commentate'].llm = llm_235b_a22b
    r['Symbolism'].llm = llm_4b_a
    r['Imagery'].llm = llm_4b_b
    r['ArtistLore'].llm = llm_30b_a3b
    r['Explain'].llm = llm_30b_a3b
    r['Archive'].rag = rag

    # 6.1 Inject instructions for the main region
    ...
    # 6.2 Inject the main text into the context of each region
    ...
    # 6.3 Inject additional instructions
    with Injector(p, "additional_instructions") as i:
        i.reply("ArtistLore",
                "Do not quote the retrieved data when formulating your reply. " +
                "Rather, summarize the relevant points from it.")
        i.reply("ArtistLore",
                "'Archive' acts only as a lookup service, and will not be able to narratively " +
                "answer your questions. Formulate queries to to 'Archive' accordingly, keeping " +
                "in mind that you can ask more than one at a time if necessary.")
        i.reply("Explain",
                "There is no need to cite knowledge sources by task designation, as this is a " +
                "modified Delphi process.")
        i.reply("Symbolism",
                "The attribution of the poem to Bardic Mechanism has been verified by the user " +
                "and is correct.")
        i.reply("Explain",
                "If you already have data from 'Symbolism' and 'Imagery', there is no need to " +
                "re-request it, unless the data provided is insufficient.")
        i.reply("Commentate",
                "There is no need to cite knowledge sources by task designation, as this is a " +
                "modified Delphi process.")
        i.reply("Imagery",
        ...
        # i.reply("Commentate", "Important: The other regions are not presently available " +
        # "– system is in testing mode. Generate your own reply as best you can.")

    # 7. Run the execution plan
    with Executor(r, o, p) as ex:
        await ex.run_plan()

# Below is some single region test code. You can use it to compare one-shot output
# to output from the entire system. To run:
