...
const REGION_CATALOG = {
  ...
  ListenerRegion: {
    ...
    defaults: (i) => ({ name: `Listener_${i}`, task: "Listen and forward specific events/messages", connections: {} }),
    methods: { 
...
function RegionNode({ data, selected }) {
  ...
  return (
    <div className={`${nodeClass} ${selected ? "ring-2 ring-indigo-500" : layerBorderClass} p-4 w-[220px] relative`}>
      {/* Layer assignment badge */}
      {isInCurrentLayer && chainName && (
        <div className={`absolute -top-2 -right-2 text-xs px-2 py-1 rounded-full ${layerInfo.chainColorClass} ${layerInfo.chainTextClass}`}>
          {chainName}
        </div>
      )}
      ...
      <div className={`text-lg font-semibold break-words ${isDarkMode ? 'text-white' : 'text-black'}`} title={params?.name}>
        {params?.name}
      </div>
      {params?.task && (
        <div className={`text-xs mt-1 break-words ${isDarkMode ? 'text-gray-200' : 'text-gray-700'}`} title={params.task}>
          {params.task}
        </div>
      )}

      {/* Input & Output handles ("pips") - attach mouse handlers to detect hover on handles too */}
      <Handle type="target" position={Position.Left} id="in" onMouseEnter={() => data.onHandleEnter?.(data.nodeId)} onMouseLeave={() => data.onHandleLeave?.()} />
      {/* Hide outgoing handle for ListenerRegion since it doesn't pass messages to other regions */}
      {typeName !== 'ListenerRegion' && (
        <Handle type="source" position={Position.Right} id="out" onMouseEnter={() => data.onHandleEnter?.(data.nodeId)} onMouseLeave={() => data.onHandleLeave?.()} />
      )}
    </div>
  );
}
...
function ParamEditor({ 
  ...
  if (!selectedNode) return <div className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Select a region to edit parameters.</div>;
...
function EditorImpl({ isDarkMode, setIsDarkMode }) {
  ...
  const onConnect = useCallback((connection) => {
    ...
    setEdges((eds) => addEdge({ ...connection, type: "default", id: `e_${connection.source}_${connection.target}` }, eds));
  ...
  const syncEdgesWithConnections = useCallback((sourceNodeId, connectionsObj) => {
    // map names -> node ids
    const nameToId = {};
    nodesRef.current.forEach((n) => { nameToId[n.data.params.name] = n.id; });
    ...
    setEdges((prevEdges) => {
      ...
      Object.keys(connectionsObj || {}).forEach((targetName) => {
        ...
        if (!exists) next.push({ id: `e_${sourceNodeId}_${targetId}`, source: sourceNodeId, target: targetId, type: "default" });
      });
  ...
  useEffect(() => {
    if (!selectedNode) { setConnectionsEditBuffer(""); return; }
  ...
  const sidebar = (
    <div className={`h-full space-y-3 ${isDarkMode ? 'dark' : ''}`}>
      ...
      <Card className={`rounded-2xl ${isDarkMode ? 'bg-gray-900 border-gray-700' : ''}`}>
        <CardHeader className="pb-2">
          <CardTitle className={`text-base ${isDarkMode ? 'text-white' : ''}`}>Chain Assignment - Layer {selectedLayer}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="space-y-2">
            {Object.entries(layerConfig[selectedLayer] || {}).map(([chainName, regions]) => {
              ...
              return (
              <div key={chainName} className={`border rounded p-2 ${isDarkMode ? 'border-gray-600 bg-gray-800' : 'border-gray-200 bg-gray-50'}`}>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className={`text-sm font-medium ${isDarkMode ? 'text-white' : ''}`}>{chainName}</span>
                    <div className={`w-4 h-4 rounded ${isDarkMode ? colorConfig.dark : colorConfig.light}`}></div>
                  </div>
                  <div className="flex items-center gap-1">
                    ...
                    >
                      <SelectTrigger className={`w-20 h-6 ${isDarkMode ? 'bg-gray-700 border-gray-600 text-white' : ''}`}>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className={isDarkMode ? 'bg-gray-800 border-gray-600' : ''}>
                        {CHAIN_COLORS.map((color, index) => (
                          <SelectItem key={index} value={index.toString()} className={isDarkMode ? 'text-white hover:bg-gray-700' : ''}>
                            <div className="flex items-center gap-2">
                              <div className={`w-3 h-3 rounded ${isDarkMode ? color.dark : color.light}`}></div>
                              {color.name}
                            </div>
                          </SelectItem>
                        ))}
          ...
          <div className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            Drag regions from the flow diagram into chains, or click a region and select "Assign to Chain" below.
          </div>
              ...
              >
                ...
                <SelectContent className={isDarkMode ? 'bg-gray-800 border-gray-600' : ''}>
                  {Object.keys(layerConfig[selectedLayer] || {}).map(chainName => (
                    <SelectItem key={chainName} value={chainName} className={isDarkMode ? 'text-white hover:bg-gray-700' : ''}>
                      {chainName}
                    </SelectItem>
                  ))}
                </SelectContent>
      ...
      <Tabs defaultValue="params" className="grow grid grid-rows-[auto,1fr]">
        <TabsList className={`w-full ${isDarkMode ? 'bg-gray-800 border-gray-600' : ''}`}>
          <TabsTrigger value="params" className={isDarkMode ? 'text-white data-[state=active]:bg-gray-700' : ''}>
            Parameters
          </TabsTrigger>
          <TabsTrigger value="methods" className={isDarkMode ? 'text-white data-[state=active]:bg-gray-700' : ''}>
            Methods
          </TabsTrigger>
          <TabsTrigger value="execution" className={isDarkMode ? 'text-white data-[state=active]:bg-gray-700' : ''}>
            Execution
          </TabsTrigger>
        </TabsList>
        <TabsContent value="params" className="overflow-auto p-0">
          <Card className={`rounded-2xl h-full ${isDarkMode ? 'bg-gray-900 border-gray-700' : ''}`}>
            <CardHeader className="pb-1">
              <CardTitle className={`text-sm ${isDarkMode ? 'text-white' : ''}`}>Init Parameters</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <ParamEditor 
              ...
              />
              <div className="flex gap-2">
                <Button variant="destructive" disabled={!selectedNodeId} className="gap-2" onClick={() => setPendingDeleteId(selectedNodeId)}>
        ...
        <TabsContent value="methods" className="overflow-auto p-0">
          <Card className="rounded-2xl h-full"><CardHeader className="pb-1"><CardTitle className="text-sm">Available Methods</CardTitle></CardHeader>
            <CardContent>
              {selectedNode ? (<MethodList typeName={selectedNode.data.typeName} />) : (<div className="text-sm text-gray-500">Select a region to see its methods and docstrings.</div>)}
            </CardContent>
          </Card>
        </TabsContent>
        <TabsContent value="execution" className="overflow-auto p-0">
          <Card className={`rounded-2xl h-full ${isDarkMode ? 'bg-gray-900 border-gray-700' : ''}`}>
            <CardHeader className="pb-1">
              <CardTitle className={`text-sm ${isDarkMode ? 'text-white' : ''}`}>Method Execution - Layer {selectedLayer}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
                ...
                {Object.entries(layerConfig[selectedLayer] || {}).map(([chainName, regions]) => (
                  <div key={chainName}>
                    ...
                    {regions.map(regionName => {
                      ...
                      return (
                        <div key={regionName} className={`border rounded p-3 ${isDarkMode ? 'border-gray-600 bg-gray-800' : 'border-gray-200 bg-gray-50'}`}>
                          <div className={`text-sm font-medium mb-2 ${isDarkMode ? 'text-white' : ''}`}>
                            {regionName}
                          </div>
                          ...
                          <div className="space-y-1 mb-2">
                            {currentMethods.map(([, method], index) => (
                              <div key={index} className="flex items-center justify-between">
                                <span className={`text-xs px-2 py-1 rounded ${isDarkMode ? 'bg-green-600 text-white' : 'bg-green-100 text-green-800'}`}>
                                  {index + 1}. {method}()
                                </span>
                                <Button 
                                  size="sm" 
                          ...
                          <div className="flex gap-2">
                            ...
                            >
                              <SelectTrigger className={`flex-1 ${isDarkMode ? 'bg-gray-700 border-gray-600 text-white' : ''}`}>
                                <SelectValue placeholder="Add method" />
                              </SelectTrigger>
                              <SelectContent className={isDarkMode ? 'bg-gray-800 border-gray-600' : ''}>
                                {availableMethods.map(method => (
                                  <SelectItem key={method} value={method} className={isDarkMode ? 'text-white hover:bg-gray-700' : ''}>
                                    {method}()
                                  </SelectItem>
                                ))}
                              </SelectContent>
              ...
              {Object.keys(layerConfig[selectedLayer] || {}).length === 0 && (
                <div className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  No regions assigned to this layer. Use the Chain Assignment panel to add regions first.
                </div>
              )}
                ...
                <div className="mt-4">
                  ...
                  <div className="space-y-1">
                    {executionConfig[selectedLayer].map(([region, method], index) => (
                      <div key={index} className={`text-xs px-2 py-1 rounded ${isDarkMode ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-800'}`}>
